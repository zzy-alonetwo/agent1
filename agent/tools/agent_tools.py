...
def _gaode_get(path: str, params: dict) -> dict:
    gaode_key = (agent_conf.get("gaode_key") or "").strip()
    if not gaode_key:
...
def generate_external_data():
    ...
    if not external_data:
        external_data_path=get_abs_path1(agent_conf["external_data_path"])
...
def fill_conent_for_report():
    return "fill_conent_for_report已调用"


"""
@tool(description='获取指定城市的天气，以纯字符串形式返回')
def get_weather(city:str)->str:
    return f"城市{city}天气为晴天，26摄氏度，空气湿度50%，南风1级，AQI21，最近6小时降雨概率极低"
